...
export default {
    ...
    data: new SlashCommandBuilder()
        .setName('cute')
        .setDescription('displays cute pet pic'),
    //functionality to run from event handler when thje command is used
    async execute(interaction) {
        await interaction.deferReply();
        const phrase = await fetchByRandomValue('phrase','phrases','type','cute');
        const file  = await getImageBufferFromBucket('photos','cute');
        //reply
